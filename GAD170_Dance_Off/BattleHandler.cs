...
public class BattleHandler:MonoBehaviour
{
    ...
    public void Battle(BriefManager player, BriefManager npc)
    {
        ...
        {
            SetWinningEffects(player, npc, 1); 
            player.levellingSystem.AddXP(10); 
            npc.levellingSystem.AddXP(0);
        }
        
        else if (playerPowerLevel < npcPowerLevel)
        {
            SetWinningEffects(player, npc, -1);
            player.levellingSystem.AddXP(2);
            npc.levellingSystem.AddXP(1);
        }
        else
        {
            SetWinningEffects(player, npc, 0);
            player.levellingSystem.AddXP(5);
            npc.levellingSystem.AddXP(2);
        }


        Debug.Log("Draw yo, need to git gud");
        // npc gets no xp either
